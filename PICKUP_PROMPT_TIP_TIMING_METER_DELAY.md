...
User said - "check screenshots a few more bugs we're alot closer. controller connection bug it doesnt work the first 5-10 times i try and connect then first 2 shots hit tip perfectly then instant lates after logs should be copy and pasteable so i can copy and paste the entire log to you. meter delay toggle doesnt work so i couldnt test it. give these to a fable workflow make sure fable PRIORITIES those abortions and LATES and TIP TIMING and METER DELAY all ship blockers and all major selling points make sure its production ready when im done. connect should work the first time especially if my controller is plugged in. no "plug in controller" bug when im already plugged in meter delay MUST be robust and foolproof and the bot must work with it entirely when its on no bugs. tip timing ABSOLUTE PRIORITY if fable can get the bot to consistiently hit the tip for majority of shots project complete excluding other bugs audit the source code make improvements heafty improvements tip timing authority so the bot always times to the tip no lates or earlies perfect timed shots always unless theres lag or jitter. we're getting closer and closer remember FABLE 5 ONLY for the workflow and they can implement the changes and debug heavy ping me when its ready again" 

- Follow this stricly ask any questions needed for clarification about anything
