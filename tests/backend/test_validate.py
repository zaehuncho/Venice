...
class TestValidate:
    ...
    def test_machine_mismatch(self, lf):
        ...
        assert status == 403
        assert body["error"] == "machine mismatch"
    ...
    def test_revoked_flag(self, lf):
        ...
        assert status == 403
        assert body["error"] == "license revoked"
    ...
    def test_invalid_key(self, lf):
        ...
        assert status == 403
        assert body["error"] == "invalid license"
    ...
    def test_killswitch(self, lf):
        ...
        status, body, _ = invoke(lf, "POST", "/api/license/check", body={
            "license_key": "ORION-VALID-BBBB-CCCC", "machine_id": "MACHINE-1"})
        assert status == 503
        assert body["error"] == "service disabled"
    ...
    def test_expired_license_rejected(self, lf):
        ...
        assert status == 403
        assert body["error"] == "license expired"

    def test_lifetime_expiry_zero_accepted(self, lf):
        put_license(lf, "ORION-LIFE-BBBB-CCCC", machine_id="MACHINE-1", expiry=0)
        status, body, _ = invoke(lf, "POST", "/api/license/check", body={
            "license_key": "ORION-LIFE-BBBB-CCCC", "machine_id": "MACHINE-1"})
        assert status == 200
        assert body["ok"] is True

    def test_legacy_row_without_expiry_still_accepted(self, lf):
        # Legacy rows with no numeric "expiry" attribute keep today's behaviour.
