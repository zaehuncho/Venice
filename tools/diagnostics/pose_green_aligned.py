"""Cue localizability + fusion measured vs the meter GREEN MOMENT (fill first >=93%) instead of the
edge. The edge (shot start) carries the whole variable rise duration; the green is the release point,
so it may be a much tighter reference. If cues/fusion are tight vs green, the pose is better-coupled
than the edge made it look.
Usage: python green_aligned_fusion.py "<video>" [count] [start] [handed]  (env METER_COLOR)
"""
import sys, os
...
while n < COUNT:
    ...
    FILL.append(float(r.fill_pct) if r.detected else np.nan)
    ...
    WY.append(float(pose._wrist_y[j])); HY.append(float(pose._hip_y[j]))
...
fill = np.array(FILL); ms = 1000.0 / fps
W = 35
cues = {"w_velpeak": [], "w_apex": [], "w_accelpeak": [], "w_snap": [], "hip_apex": []}
for e in shot_edges:
    lo, hi = e, min(len(wy), e + W)
    seg = fill[lo:hi]
    gi = np.where(seg >= 93.0)[0]
    if len(gi) == 0 or hi - lo < 6:
        continue
    gm = lo + int(gi[0])                       # GREEN moment = first fill>=93
    apex = lo + int(np.argmin(wy[lo:hi]))
    snap = apex + (int(np.argmax(wv[apex:hi])) if apex < hi - 1 else 0)
    cues["w_velpeak"].append((lo + int(np.argmin(wv[lo:hi])) - gm) * ms)
    cues["w_apex"].append((apex - gm) * ms)
    cues["w_accelpeak"].append((lo + int(np.argmax(np.abs(wa[lo:hi]))) - gm) * ms)
    cues["w_snap"].append((snap - gm) * ms)
    cues["hip_apex"].append((lo + int(np.argmin(hy[lo:hi])) - gm) * ms)
...
names = list(cues); nrec = len(cues["w_apex"])
print(f"VIDEO {os.path.basename(VIDEO)} shots_with_green={nrec}  (reference = GREEN moment)")
meds = {}
for nm in names:
