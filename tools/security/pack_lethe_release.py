...
SERVER_SHARD_RELEASE_BLOCKER = (
    "server-shard release is blocked: the signed outer activation broker that "
    "creates the DPAPI shard_session.dat before the bootstrap runs is not "
    "implemented. Refusing to emit a customer package that cannot pass a fresh "
    "install. See docs/SERVER_SHARD_2026-09-19.md."
)


def validate_server_shard_release_mode(enabled: bool) -> None:
    """Fail before copying or signing while the first-launch broker is absent."""
    if enabled:
...
def validate_release_targets(targets: list[str]) -> None:
    ...
    Lethe's internal DLL round-trip harness remains useful lab coverage,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "When omitted, defaults to the standalone Lethe CLI "
            "(LETHE_CLI env, else C:\\Users\\aaron\\Desktop\\Lethe\\lethe.py)."
        ),
    )
    ...
    parser.add_argument(
        "--server-shard",
        action="store_true",
        help=("Request the release server-shard path. This currently fails "
              "closed until the signed first-launch activation broker exists."),
    )
    args = parser.parse_args(argv)

    try:
        validate_server_shard_release_mode(args.server_shard)
    except ValueError as exc:
        parser.error(str(exc))

    if not args.verify_only and not args.packer_command:
        # Default to the standalone Lethe CLI so releases pack out of the box.
        lethe_cli = Path(DEFAULT_LETHE_CLI)
        if lethe_cli.is_file():
            args.packer_command = (
                f'"{sys.executable}" "{lethe_cli}" {{input}} {{output}}'
            )
            print(f"[orion-pack] no --packer-command given; defaulting to Lethe CLI: {lethe_cli}")
        else:
            parser.error(
                "--packer-command is required unless --verify-only is set "
                f"(Lethe CLI not found at {lethe_cli}; set LETHE_CLI or pass --packer-command)"
            )
