...
physical Square edge -- and the sidecar's own first sight is ~36 ms EARLIER than the
engine's accept (measured 2026-09-16), so this module converts before bucketing:

    onset_engine_ms = onset_ms + ORION_SHOT_RECORD_ONSET_LAG_MS   (default 36)

Both numbers are written to the record, along with the lag that was used, so a later
re-calibration of the lag can re-bucket the whole corpus offline without re-collecting.
...
class ShotRecorder:
    ...
    def create(cls, session='', log=None, env=None, start_thread=True, clock=None):
        ...
        try:
            ...
            if not explicit and (env.get('PYTEST_CURRENT_TEST') or 'pytest' in sys.modules):
                return None
            root = str(env.get('ORION_SHOT_RECORDS_ROOT', '') or '').strip()
            if not root:
                # C: runs at ~7 GB free on this workstation; every new output goes to D:.
                root = os.path.join('D:\\', 'NexusVision', 'shot_records')
    ...
    def stop(self, timeout=2.0):
        """Close every open record, drain the queue, retire the writer."""
        try:
        ...
        self._thread = None
        # Whatever the writer did not reach is written here rather than lost.
    ...
    def note_pickup(self, epoch, pickup):
        ...
        with self._lock:
            ...
            onset = keep.get('first_sight_ms_after_press')
            # -1.0 is the reader's "this press never picked a meter up" sentinel.
            if onset is not None and onset >= 0.0:
                rec['onset_ms'] = onset
                rec['onset_source'] = 'pickup'
                fill = keep.get('first_sight_fill')
    ...
    def note_onset(self, epoch, onset_ms, fill=None, source='detect_loop'):
        """Fallback onset: the sidecar's own first accepted sample for this press.

        The PICKUP record is authoritative when it exists (it is the number the +36 ms
        engine-lag calibration was measured against); this only fills the gap for a
        proposer that keeps no pickup record.
        """
        ep = self._epoch(epoch)
        val = _num(onset_ms)
        ...
        with self._lock:
            rec = self._open.get(ep)
            if rec is None or rec.get('onset_source') == 'pickup':
                return False
            rec['onset_ms'] = round(val, 2)
    ...
    def _recompute_tempo(self, rec):
        ...
        if onset is None or onset < 0.0:
            ...
            rec['onset_engine_ms'] = round(engine, 2)
            rec['tempo'] = tempo_for(shot_type, engine)
    ...
    def _finish(self, epoch, reason, now_ms):
        ...
        try:
            ...
            self._log.error('SHOT RECORD DROPPED: epoch=%d reason=%s (writer queue full, '
                            'dropped=%d)', rec['epoch'], reason, self.records_dropped)
    ...
    def drain(self, block=False):
        """Write everything queued right now.  Tests run the recorder without a thread."""
        n = 0
        while True:
            try:
